...
;;;	windows.el‚ ig‚Á‚ ¢‚é ‚Í revive.el ‚ ½‚  L[‚ „‚äí
;;;	‚ àautoload‚  è‚à‚·‚é•K—v‚ª‚ ‚è‚ ¹‚ñB
;;;--------------------------------------------------------------------------
;;;
...
;;;yrevive‚  z
;;;
;;;	  revive.el ‚ğg‚¤‚ AEmacs g—p‚ ó‘ ğƒtƒ@ƒCƒ‹‚ Z[ƒu‚µ‚ A
;;;	Ÿ‰ñ Emacs ‚ğ‹N“®‚·‚ ‚ »‚ ó‘  œ‹A‚·‚ç·‚ ª‚ «‚ ·B‚à‚¿
;;;	‚ë‚ñƒEƒBƒ“ƒhƒE‚ ªŠ„ó‘ à•œŒ³‚³‚ê‚é‚ Å saveconf ‚â desktop ‚Å
;;;	‚¢‚ ‚ ‚ ¢‚½l‚ à‚¨Š©‚  ·B
;;;
;;;y‘g‚ ‚ ûz
;;;
;;;	  revive.el ‚ğ load-path ‚  Á‚½ƒfƒBƒŒƒNƒgƒŠ‚ ü‚êA~/.emacs ‚É
;;;	ˆ º‚ Lq‚ğ“ü‚ê‚ ­‚¾‚³‚¢B
;;;
;;;	  (autoload 'save-current-configuration "revive" "Save status" t)
;;;	  (autoload 'resume "revive" "Resume Emacs" t)
...
;;;yg‚¢•ûz
;;;
;;;	  ã‚Ì define-key ‚ğ‚µ‚½ ‚  AC-x S ‚ »   Wó‘ ğƒZ[ƒu
;;;	‚·‚ç·‚ ª‚ «‚ ·Bsave-current-configuration Š ”‚ ”ˆø”‚ğ‚Â
;;;	‚¯‚é‚ ¡”‚ ó‘ ğ• X‚ Z[ƒu‚ «‚ ·BuC-u 2 C-x Sv‚ ·‚é‚Æ2
;;;	”  oƒbƒtƒ@‚ »ó‚ğƒZ[ƒu‚ «‚ ·B‚±‚ê‚ğƒ[ƒh‚·‚ ‚à“¯—l‚É
;;;	uC-u 2 C-x Fv‚ ^ƒCƒv‚·‚é‚Æ2”  oƒbƒtƒ@‚©‚çó‘ ğƒ[ƒh‚µ‚ ·B
;;;
;;;y• ”‚ Z[ƒuz
;;;
;;;	  • ”‚ l‚àƒZ[ƒu‚µ‚ ¨‚­‚±‚ ª‚ «‚ ·BƒfƒtƒHƒ‹ƒg‚ Z[ƒu‚·‚é 
;;;	global • ”‚Í revive:save-variables-global-default ‚ Alocal •Ï
;;;	”‚Í revive:save-variables-local-default ‚ è‹`‚³‚ê‚ ¢‚ ·B‚Ù
;;;	‚©‚  ”‚à• ¶‚µ‚½‚¢ ‚ Arevive:save-variables-global-private 
;;;	‚É global • ”–¼‚ğArevive:save-variables-local-private ‚É local 
;;;	• ”–¼‚ğ‚»‚ ‚ ƒXƒg‚ `‚ è‹`‚µ‚ ¨‚«‚ ·B— ‚Î gmhist ‚ğg‚Á
;;;	‚ ¢‚é ‚  A
;;;
;;;		(setq revive:save-variables-global-private
;;;		      '(file-history buffer-history minibuffer-history))
...
;;;y•’   ¢ƒoƒbƒtƒ@‚ µ‚¢z
;;;
;;;	  mh-rmail ‚  JƒŒƒ“ƒgƒoƒbƒtƒ@‚ª mh-folder-mode, gnus ‚  JƒŒ
;;;	ƒ“ƒgƒoƒbƒtƒ@‚ª gnus-Group-mode ‚  è‚ ·B‚±‚  Š W‚ A• ” 
;;;	revive:major-mode-command-alist-default ‚ ‘‚©‚ê‚ ¢‚ ·B‚±‚ Ï
;;;	”‚ o˜^‚³‚ê‚ ¢‚éˆ O‚ à‚ ğ’è‹`‚µ‚½‚¢ ‚ A
;;;
;;;		(setq revive:major-mode-command-alist-private
;;;		  '((hogehoge-mode	. hoge)
...
;;;		    ("ƒ^ƒCƒv•ƒƒjƒ…["	. trr)))
;;;
;;;	‚  ‚É revive:major-mode-command-alist-private ‚ l‚ğ  ‚é
;;;	‚ Ÿ‰ñ resume ‚µ‚½‚ ©“®“I‚  ‚·‚éƒRƒ}ƒ“ƒh‚ª‹N“®‚³‚ê‚ ·B‚Ü
;;;	‚½ã‚ á‚  ‚é‚ ‚ Amajor-mode(ƒVƒ“ƒ{ƒ‹)‚ ã‚í‚è‚É buffer-name
;;;	(•¶š—ñ)‚ğw’ ‚ç·‚ à‚ «‚ ·B
;;;
;;;	  ‚ ½ASKK‚ «‘‚  ‚ ŠƒWƒ…[ƒ€‚·‚é‚ ¤‚ ­“®‚©‚ ­‚ Á‚ µ
;;;	‚ ¤ƒoƒbƒtƒ@‚ª‚ ‚é ‚ A• ” revive:ignore-buffer-pattern ‚ »
;;;	‚ oƒbƒtƒ@–¼‚ªƒ}ƒbƒ`‚·‚é‚ ‚ ³‹K•\Œ»‚ğ  ‚ ­‚¾‚³‚¢B
;;;
;;;yƒvƒƒOƒ‰ƒ€‚©‚çg‚¤z
;;;
;;;	  ‰pŒê” hƒLƒ…ƒƒ“ƒg [For programmers] ‚ €‚ğQ µ‚ ­‚¾‚³‚¢B
;;;
;;;y‚ ‚ ª‚«z
;;;
;;;	   ‰‚Í resume ‚ ¢‚¤ƒtƒ@ƒCƒ‹–¼‚¾‚Á‚½‚  ·‚ªAEmacs 19 ‚ fƒB
;;;	ƒŒƒNƒgƒŠ‚É resume.el ‚ ¢‚¤ƒtƒ@ƒCƒ‹‚ª‚ ‚Á‚ Vƒ‡ƒbƒN‚ğ  µ‚½B
;;;	‚±‚¿‚ç‚ Rƒ}ƒ“ƒhƒ‰ƒCƒ“‚ ½‰ñ emacs ‚  Á‚ àAŠù‚ N“®‚µ‚ ¢‚é 
;;;	emacs ‚ tƒ@ƒCƒ‹‚ğ“n‚·‚ ¢‚¤‚¾‚¯‚Ì(ƒs[[)ƒvƒƒOƒ‰ƒ€‚ u‚ ±‚ª 
;;;	resume ‚â‚ ñv‚ ¾‚¢‚½‚­‚ è‚ µ‚½‚ª‰ ‚µ‚Ä revive.el ‚ Šƒl[
;;;	ƒ€‚µ‚ µ‚½B‚ ‚ ‚ Á‚½‚­Asaveconf ‚ à desktop ‚ à‚ µ“¾‚ ©‚Á
;;;	‚½ƒEƒBƒ“ƒhƒE•ªŠ„ó‘  œŒ³‚ğƒTƒ|[ƒg‚µ‚½‚ ¾‚¤‚  cA‚ ñ‚ ±‚Æ
;;;	‚ pŒê”   ‘‚¯‚ ¢‚È:-)B
;;;
;;;yæ‚‘Ç‚¢z
;;;
;;;	  ‚±‚ vƒƒOƒ‰ƒ€‚ AƒtƒŠ[ƒ\ƒtƒgƒEƒFƒA‚ ¢‚½‚µ‚ ·B‚±‚ vƒƒOƒ‰
;;;	ƒ€‚ğg—p‚µ‚ ¶‚¶‚½‚¢‚©‚  ‰   µ‚ àì  ê   C‚ í‚È
;;;	‚¢‚à‚  ¢‚½‚µ‚ ·‚ªAƒRƒƒ“ƒg‚âƒoƒOƒŒƒ|[ƒg‚  ‚ ½Œ}‚¢‚½‚µ‚Ü
;;;	‚·B‚¨‹CŒy‚ ²˜A—‰º‚³‚¢B˜A—‚  º‚ AƒhƒŒƒX‚  ¨Šˆ‚¢‚½‚µ‚Ü
;;;	‚·(2003/6Œ»İ)B
;;;							yuuji@gentei.org
